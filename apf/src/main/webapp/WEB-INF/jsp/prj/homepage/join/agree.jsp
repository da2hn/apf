...
부칙
이 약관은 2009년 1월 1부터 적용됩니다.--%><h3>이용약관</h3>

제1조. 동의 
경기도노인종합상담센터에서 운영하는 경기도노인종합상담센터 인터넷 
서비스(https://www.gg.go.kr/gnoincoun/ 이하 경기도노인종합상담센터)는 이 약관에 
동의한 이용자들에게 무료로 제공되는 서비스입니다. 이용자가 등록절차를 거쳐서 
"회원가입" 단추를 누르면 이 약관에 동의하였음을 협약 합니다.

•제2조 약관의 적용 
서비스 이용에 관하여는 이 약관을 적용하며, 이 약관에 명시되지 아니한 사항에 대하여는 
전기통신기본법, 전기통신사업법, 정보통신망이용촉진 등에 관한 법률 및 기타 관계법령의 
규정에 의합니다.

•제3조. 회원가입
① 경기도노인종합상담센터 회원은 ‘본인회원‘을 원칙으로 합니다.
② ‘본인회원’이란 이 규약을 승인하고 경기도노인종합상담센터 회원신청 양식에 의거, 본인
의 신상 정보를 사실에 입각, 입력하여 ID와 Password를 발급받으신 분을 의미합니다.
③ 경기도노인종합상담센터는 다음 사항에 해당하는 경우에는 가입승인을 유보 또는 거부할 
수 있습니다.
1. 가명으로 회원에 가입할 경우.
2. 혐오스럽거나 타인에게 불쾌감을 줄 수 있는 ID로 회원가입을 할 경우.
3. 다른 사람의 명의를 빌리거나 도용하여 차명으로 신청하였을 경우.
4. 기타 가입신청 항목에 허위로 기재할 경우.
5. 관계법령에 위배되거나 미풍양속을 저해하고자 하는 목적으로 회원가입 신청을 할 경우.

•제4조. 고객 정보 이용 
경기도노인종합상담센터는 이용자의 정보를 집단적인 형태로 사용할 수는 있지만 각 이용
자 개인정보는, 불법적이거나 기타 다른 불온한 문제를 일으킬 경우를 제외하고, 이용자의 
동의 없이는 공개하지 않습니다.
또한 경기도노인종합상담센터에 기재되는 모든 이용자 정보는 이름을 포함하여 모두 실제 
데이터인 것으로 간주됩니다.
실명이나 실제 정보를 입력하지 않은 사용자는 법적인 보호를 받을 수 없으며 경기도노인종
합상담센터 서비스 제한 조치를 받을 수 있습니다.
개인 정보의 변경의무는 전적으로 사용자에게 책임이 있으며 변경 지연으로 인해 발생된 손
실에 대해 경기도노인종합상담센터에서는 책임지지 않습니다.
•제5조. 이용고객의 의무 
회원 가입 시에 요구되는 귀하에 대한 정보는 진실하고 정확하여야 합니다.
타 회원을 통해서 받는 서비스나 당사 제휴의 서비스가 아닌 기타 사이트에서 제공하는 서
비스에 대해서는 내용상의 정확성, 완전성 및 질에 대하여 경기도노인종합상담센터는 책임
을 지지 않습니다.
따라서 당사는 고객이 위 내용을 이용함으로 인하여 입게 된 모든 종류의 손실이나 손해에 
대하여 책임을 지지 아니합니다.
귀하는 본 서비스를 통하여 다음과 같은 행동을 하지 않는데 동의하며 이를 어길 경우 당사
는 게시물을 사전통보 없이 삭제할 수 있으며 회원 권한에 제재를 가할 수 있음을 알려 드
립니다. 또한, 그로인해 당사의 물질적 손해를 발생시켰을 경우에는 해당손해를 변상하여야 
할 의무가 있습니다.
① 타인의 ID와 Password를 도용하는 행위
② 저속, 음란, 모욕적, 위협적이거나 타인의 프라이버시를 침해할 수 있는 내용을 전송, 게
시, 게재, 전자 메일 또는 기타의 방법으로 전송하는 행위
③ 다른 사용자의 개인정보를 수집 또는 저장하는 행위
④ 당사의 서버를 해킹 또는 기타방법을 이용하여 특정혜택 또는 이익을 취득하는 행위

•제6조. 회원가입 시의 ID와 비밀번호 
고객ID와 비밀번호에 관한 모든 관리책임은 고객에게 있습니다.
고객에게 부여된 고객ID 및 비밀번호의 관리소홀, 부정사용에 의하여 발생하는 모든 결과에 
대한 책임은 고객에게 있습니다.
고객의 ID가 부정하게 사용된 경우 고객은 반드시 경기도노인종합상담센터에 그 사실을 통
보해야 합니다.

•제7조. 경기도노인종합상담센터 서비스 
당사는 본 약관을 사전 고지 없이 변경할 수 있으며, 변경된 약관은 초기 화면에 게재하거
나 기타 다른 방법으로 이용자에게 공지함으로써 효력을 발생합니다.
가입자가 변경된 약관에 동의하지 아니하는 경우, 가입자는 본인의 회원 등록을 취소할 수 
있으며, 계속 사용의 경우는 약관 변경에 대한 동의로 간주됩니다.
또한 경기도노인종합상담센터에서는 불가피한 사정으로 인하여 서비스를 수정하거나 중단할 
수 있으며 다음과 같은 경우에 당사는 책임을 지지 않습니다.
① 본 서비스에 보관되거나 전송된 메시지 및 기타 통신 메시지 등의 내용이 국가의 비상사
태, 정전, 당사의 관리범위 외의 서비스 설비장애 및 기타 불가항력(바이러스)에 의하여 보
관되지 못하였거나 삭제된 경우, 전송되지 못한 경우 및 기타 통신 데이터의 손실이 있을 
경우
② 당사가 정상적인 서비스 제공의 어려움으로 인하여 일시적으로 서비스를 중지하여야 할 
경우에 서비스 중지 1주일 전의 고지 후 서비스를 중지할 수 있으며, 이 기간 동안 귀하가 
공지내용을 인지하지 못한 데 대하여 당사는 책임을 부담하지 않습니다.
피치 못할 사정이 있을 경우 위 사전 고지기간은 감축되거나 생략될 수 있습니다.
또한 위 서비스 중지에 의하여 본 서비스에 보관되거나 전송된 메시지 및 기타 통신 메시지 
등의 내용이 보관되지 못하였거나 삭제된 경우, 전송되지 못한 경우 및 기타통신 데이터의 
손실이 있을 경우에 대하여도 당사는 책임을 부담하지 아니합니다.
위와 같이 경기도노인종합상담센터는 서비스 이용과 관련하여 이용자에게 발생한 어떠한 손
해에 관하여도 책임을 지지 않으며 서비스 이용으로 발생 소송이 제기될 경우 경기도노인종
합상담센터의 소재지를 관할하는 법원을 관할 법원으로 합니다.

•[ 부 칙 ]    
(시행일) 이 약관은 2024년 05월 30일부터 시행합니다.
          </div>
          <div style="width: 100%;">
...
◆ 경기도노인종합상담센터는 회원가입, 게시판 이용, 상담 등을 위해 아래와 같이 개인정
보를 수집·이용합니다.
